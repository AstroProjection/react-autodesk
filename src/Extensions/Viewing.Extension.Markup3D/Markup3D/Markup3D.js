...
import EventsEmitter from '../Extras/EventsEmitter';
...
export default class Markup3D extends EventsEmitter {
  ...
  constructor(viewer, screenPoint, dbId, fragId, worldPoint, opts = {}) {
    ...
    this.initialMeshPos = this.meshPosition(this.fragId);

    this.initialWorldPoint = worldPoint || this.screenToWorld(screenPoint);

    this.pinMarker = new PinMarker(viewer, this.initialWorldPoint);

    this.onTrackerModified = throttle(this.onTrackerModified, 10);

    this.trackerModifiedHandler = (screenPoint) => this.onTrackerModified(screenPoint);

    this.pinMarker.on('tracker.modified', this.trackerModifiedHandler);
    ...
    this.leaderContainer = $('.leader-container')[0];

    this.onMouseUpHandler = (event) => this.onMouseUp(event);

    $(viewer.container).on('mouseup', this.onMouseUpHandler);
    ...
    this.leader = new Leader(this.leaderContainer, this.startPoint);

    this.labelMarker = new LabelMarker(this, this.viewer, this.dbId, this.startPoint, opts.properties);
