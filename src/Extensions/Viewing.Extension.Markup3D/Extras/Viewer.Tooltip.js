...
export default class ViewerTooltip extends EventsEmitter {
  ...
  constructor(viewer, opts = {}) {
    ...
    this.pointer = this.createPointer($(`#${this.svgId}`)[0]);
  ...
  activate() {
    if (!this.active || this.timeout) {
      this.viewer.container.addEventListener('mouseenter', this.onMouseEnter);

      this.viewer.container.addEventListener('mouseleave', this.onMouseLeave);
  ...
  deactivate(delay = 0) {
    if (this.active && !this.timeout) {
      this.viewer.container.removeEventListener('mouseenter', this.onMouseEnter);
