...
import ExtensionBase from './Extras/Viewer.ExtensionBase';
import ViewerTooltip from './Extras/Viewer.Tooltip';
...
class Markup3DExtension extends ExtensionBase {
  ...
  constructor(viewer, options) {
    ...
    this.markup3DTool = new Markup3DTool(viewer, this.markupCollection, options);
    ...
    this._viewer.toolController.registerTool(this.markup3DTool);

    var toolName = this.markup3DTool.getName();

    this._viewer.toolController.activateTool(toolName);

    this.onExplodeHandler = (e) => this.onExplode(e);

    this.onVisibilityHandler = (e) => this.onVisibility(e);
  }
  ...
  onToolbarCreated() {
    this._control = ViewerToolkit.createButton('toolbar-markup3D', 'glyphicon glyphicon-check', 'Markup 3D', () => {
    ...
    this.eventHandlers.forEach((entry) => {
      this._viewer.addEventListener(entry.event, entry.handler);
    });
  }
  ...
  unload() {
    this.eventHandlers.forEach((entry) => {
      if (entry.removeOnDeactivate) {
        this._viewer.removeEventListener(entry.event, entry.handler);
      }
    });
